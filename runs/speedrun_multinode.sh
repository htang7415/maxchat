...
if [ -z "${NANOCHAT_CACHE_NAMESPACE:-}" ]; then
    if [ -n "${SLURM_JOB_ID:-}" ]; then
        NANOCHAT_CACHE_NAMESPACE="job_${SLURM_JOB_ID}"
    else
        NANOCHAT_CACHE_NAMESPACE="manual_${USER}"
    fi
fi
export NANOCHAT_CACHE_NAMESPACE
...
log "NANOCHAT_CACHE_NAMESPACE=$NANOCHAT_CACHE_NAMESPACE"
log "TMPDIR=$TMPDIR"
log "TORCHINDUCTOR_CACHE_DIR=$TORCHINDUCTOR_CACHE_DIR"
log "TRITON_CACHE_DIR=$TRITON_CACHE_DIR"
log "TORCHINDUCTOR_FORCE_DISABLE_CACHES=$TORCHINDUCTOR_FORCE_DISABLE_CACHES"
log "TORCHINDUCTOR_SHAPE_PADDING=$TORCHINDUCTOR_SHAPE_PADDING"
log "TORCHINDUCTOR_COMPILE_THREADS=$TORCHINDUCTOR_COMPILE_THREADS"
