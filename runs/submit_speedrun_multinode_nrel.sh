...
export NANOCHAT_CACHE_NAMESPACE="${NANOCHAT_CACHE_NAMESPACE:-job_${SLURM_JOB_ID}}"
export TMPDIR="${TMPDIR:-$NANOCHAT_BASE_DIR/tmp}"
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-$NANOCHAT_BASE_DIR/torchinductor_cache/${NANOCHAT_CACHE_NAMESPACE}}"
export TRITON_CACHE_DIR="${TRITON_CACHE_DIR:-$NANOCHAT_BASE_DIR/triton_cache/${NANOCHAT_CACHE_NAMESPACE}}"
# Conservative defaults to avoid known multi-process inductor cache corruption.
export TORCHINDUCTOR_FORCE_DISABLE_CACHES="${TORCHINDUCTOR_FORCE_DISABLE_CACHES:-1}"
export TORCHINDUCTOR_SHAPE_PADDING="${TORCHINDUCTOR_SHAPE_PADDING:-0}"
...
echo "NANOCHAT_CACHE_NAMESPACE=${NANOCHAT_CACHE_NAMESPACE}"
echo "TMPDIR=${TMPDIR}"
echo "TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR}"
echo "TRITON_CACHE_DIR=${TRITON_CACHE_DIR}"
echo "TORCHINDUCTOR_FORCE_DISABLE_CACHES=${TORCHINDUCTOR_FORCE_DISABLE_CACHES}"
echo "TORCHINDUCTOR_SHAPE_PADDING=${TORCHINDUCTOR_SHAPE_PADDING}"
...
srun \
  ...
  --ntasks-per-node=1 \
  --kill-on-bad-exit=1 \
  --export="ALL,TMPDIR=${TMPDIR},TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR},TRITON_CACHE_DIR=${TRITON_CACHE_DIR},NANOCHAT_BASE_DIR=${NANOCHAT_BASE_DIR},NANOCHAT_CACHE_NAMESPACE=${NANOCHAT_CACHE_NAMESPACE},TORCHINDUCTOR_FORCE_DISABLE_CACHES=${TORCHINDUCTOR_FORCE_DISABLE_CACHES},TORCHINDUCTOR_SHAPE_PADDING=${TORCHINDUCTOR_SHAPE_PADDING},TORCHINDUCTOR_COMPILE_THREADS=${TORCHINDUCTOR_COMPILE_THREADS}" \
  --chdir="$WORKDIR" \
